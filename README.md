...
Минусы решения:
- Регулярные выражения сложны и капризны. В случае отклонения запрса от шаблона нужно будет добавлять новые
регулярные выражения.
- Неправильная классификация запросов - LLM ошибается, и в процессе задания не раз пришлось с этим столкнуться.
- Сложно обрабатывать объемные запросы.
